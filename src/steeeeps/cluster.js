...
define("steeeeps/cluster", ["dojo/_base/declare", "dojo/_base/lang", "esri/layers/GraphicsLayer", "esri/graphic", "esri/geometry/Multipoint", "esri/geometry/Extent", "esri/symbols/TextSymbol", "esri/symbols/SimpleMarkerSymbol", "esri/symbols/SimpleLineSymbol", "dojo/_base/Color", "esri/geometry/screenUtils", "esri/geometry/webMercatorUtils"], function(declare, lang, GraphicsLayer, Graphic, Multipoint, Extent, TextSymbol, SimpleMarkerSymbol, SimpleLineSymbol, Color, screenUtils, webMercatorUtils) {
	var ClusterGraphic = declare(Graphic, {
		...
		_updateTextSymbol : function(count) {
			if (!this._textSbl) {
				this._textSbl = new TextSymbol(2).setOffset(0, -5);
				this._textGra = new Graphic(this._clusterCenter, this._textSbl);
		...
		_defaultClassifySymols : function() {
			return {
				less10 : new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 25, null/*new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,  new Color([255, 250, 250, 0.8]), 2)*/, new Color([46, 223, 163, 0.8])),
				less25 : new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 35, null/*new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,  new Color([255, 250, 250, 0.8]), 2)*/, new Color([0, 191, 255, 0.8])),
				less50 : new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 35, null/* new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,  new Color([255, 250, 250, 0.8]), 2)*/, new Color([30, 144, 255, 0.8])),
				over50 : new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 50, null/*new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,  new Color([255, 250, 250, 0.8]), 2)*/, new Color([255, 0, 0, 0.8]))
			};
		}
	});

	var ClusterLayer = declare(GraphicsLayer, {
		constructor : function(params) {
			lang.mixin(this, params);
			if (!lang.isArray(this.esriGraphics))
				this.esriGraphics = [];

			dojo.connect(this.map, 'onZoomStart', lang.hitch(this, this._handleMapZoomStart));
			dojo.connect(this.map, 'onExtentChange', lang.hitch(this, this._handleMapExtentChange));
		...
		_handleMouseOver : function(event) {
			...
			type == ClusterLayer.GRAPHIC_TYPE_FIRSTLEVELCLUSTER && this._clusterMouseOverHandle(event);
		},
		/*
		 * graphic鼠标移出响应
		 */
		_handleMouseOut : function(event) {
			var g = event.graphic;
			var type = this._getGraphicType(g);
			type == ClusterLayer.GRAPHIC_TYPE_FIRSTLEVELCLUSTER && this._clusterMouseOutHandle(event);
		},
		/*
		 * graphic鼠标单击响应
		 */
		_handleMouseClick : function(event) {
			var g = event.graphic;
			var type = this._getGraphicType(g);
			type == ClusterLayer.GRAPHIC_TYPE_FIRSTLEVELCLUSTER && this._clusterClickHandle(event);
			type == ClusterLayer.GRAPHIC_TYPE_ESRIGRAPHIC && this._graphicClickHandle(event);
		},
		/*
		 * graphic默认符号
		 */
		...
		_getGraphicType : function(graphic) {
			if (graphic.symbol.type == 'textsymbol' || graphic.symbol.type == 'simplelinesymbol')
				return ClusterLayer.GRAPHIC_TYPE_OTHERGRAPHIC;
			if (graphic.isCluster && graphic.isFirstClusterLevel())
				return ClusterLayer.GRAPHIC_TYPE_FIRSTLEVELCLUSTER;
			if (graphic.isCluster)
				return ClusterLayer.GRAPHIC_TYPE_CLUSTER;
			else
				return ClusterLayer.GRAPHIC_TYPE_ESRIGRAPHIC;
		},
